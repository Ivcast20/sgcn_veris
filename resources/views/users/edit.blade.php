            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="department_id" name="department_id">
                        @foreach ($departamentos as $depart)
                            <option value="{{ $depart->id }}" @selected(old('department_id',$user->department_id) == $depart->id)>{{ $depart->name }}</option>
                        @endforeach
                    </select>
                    @error('department_id')
                        <span class="text-danger">{{ $message }}</span>
                ...
                <select name="roles[]" id="multipleselect" multiple class="form-control">
                    @foreach ($roles as $key => $rol)
                        <option value="{{ $rol->id }}" @selected(old('roles[]', $user->roles->contains($rol->id)))>{{ $rol->name }}
                        </option>
                    @endforeach
                </select>
                @error('roles')
